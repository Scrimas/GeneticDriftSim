...
class DriftGUI:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.pop_size_var: tk.IntVar = tk.IntVar(value=400)
        self.num_species_var: tk.IntVar = tk.IntVar(value=10)
        ...
        self.mutation_rate_var: tk.DoubleVar = tk.DoubleVar(value=0.01)
        ...
        self.num_gens_to_run: tk.IntVar = tk.IntVar(value=50)
        self.gen_count_var: tk.StringVar = tk.StringVar(value="Gen: 0")
        self.stop_on_fixation_var: tk.BooleanVar = tk.BooleanVar(value=True)
    ...
    def setup_ui(self) -> None:
        control_frame: ttk.Frame = ttk.Frame(self.root, padding="15")
        control_frame.pack(side=tk.LEFT, fill=tk.Y)

        plot_frame: ttk.Frame = ttk.Frame(self.root, padding="10")
        ...
        mutation_frame: ttk.LabelFrame = ttk.LabelFrame(control_frame, text="Mutation", padding="5")
        mutation_frame.pack(fill='x', pady=5)
        
        self.mut_check: ttk.Checkbutton = ttk.Checkbutton(mutation_frame, text="Enable Mutation", variable=self.mutation_enabled_var, command=self.on_mutation_toggle)
        self.mut_check.pack(pady=5)

        ttk.Label(mutation_frame, text="Mutation Rate:").pack(pady=2)
        self.mut_entry: ttk.Entry = ttk.Entry(mutation_frame, textvariable=self.mutation_rate_var)
        ...
        delay_frame: ttk.Frame = ttk.Frame(control_frame)
        delay_frame.pack(fill='x', pady=5)
        ttk.Label(delay_frame, text="Delay (ms):").pack(side=tk.LEFT)
        
        self.delay_entry: ttk.Entry = ttk.Entry(delay_frame, textvariable=self.speed_var, width=8)
        ...
        self.canvas: FigureCanvasTkAgg = FigureCanvasTkAgg(self.fig, master=plot_frame)
        self.canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)

        self.colors: Any = plt.get_cmap('tab20').colors
        self.grid_colors: list[str] = ['#eeeeee'] + list(self.colors)
        ...
        self.img_grid: Any = None
        self.lines_graph: dict[int, Any] = {}
    ...
    def reset_sim(self) -> None:
        ...
        num_species: int = min(self.num_species_var.get(), 20)
    ...
    def refresh_plots(self) -> None:
        ...
        grid_data: np.ndarray = np.full(side * side, -1, dtype=int)
        ...
        history: np.ndarray = self.sim.get_history_array()
        gens: np.ndarray = np.arange(history.shape[0])
        
        for s in range(20):
            counts: np.ndarray = history[:, s]
    ...
    def check_fixation(self) -> bool:
        ...
        if not self.sim or not self.stop_on_fixation_var.get():
            return False
        counts: np.ndarray = self.sim.get_counts()
