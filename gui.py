...
class DriftGUI:
    ...
    def setup_ui(self) -> None:
        ...
        plot_container = ctk.CTkFrame(self.root, fg_color=self.card_bg, corner_radius=12)
        plot_container.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, padx=(0, 20), pady=20)
        ...
        ctk.CTkLabel(mut_card, text="Mutation Dynamics", font=self.font_header, text_color=self.text_main).pack(pady=(15, 10), padx=15, anchor="w")
        
        self.mut_check = ctk.CTkSwitch(mut_card, text="Enable Mutation", font=self.font_label, text_color=self.text_main, variable=self.mutation_enabled_var, command=self.on_mutation_toggle)
        self.mut_check.pack(pady=(0, 10), padx=15, anchor="w")

        ctk.CTkLabel(mut_card, text="Mutation Rate", font=self.font_label, text_color=self.text_sub).pack(pady=0, padx=15, anchor="w")
        self.mut_entry = ctk.CTkEntry(mut_card, textvariable=self.mutation_rate_var, border_width=1, border_color=self.border_color, text_color=self.text_main, corner_radius=6)
        self.mut_entry.pack(pady=(5, 15), padx=15, fill=tk.X)
        self.on_mutation_toggle()

        # --- 3rd Card: Execution Controls ---
        ctrl_card = ctk.CTkFrame(control_sidebar, corner_radius=8, fg_color=self.card_bg)
        ...
        ctk.CTkLabel(delay_frame, text="Delay Speed", font=self.font_label, text_color=self.text_main).pack(side=tk.LEFT)
        ctk.CTkLabel(delay_frame, textvariable=self.delay_label_var, font=self.font_label, text_color=("#1F6AA5", "#569CD6")).pack(side=tk.RIGHT)
        
        ctk.CTkSlider(ctrl_card, from_=1, to=1000, variable=self.speed_var).pack(fill=tk.X, padx=15, pady=(0, 15))
        
        # Action Buttons
        ctk.CTkButton(ctrl_card, text="Start / Pause", font=self.font_header, command=self.toggle_run, height=40).pack(pady=5, padx=15, fill=tk.X)
        ctk.CTkButton(ctrl_card, text="Step 1 Gen", fg_color="transparent", border_width=1, border_color=self.border_color, text_color=self.text_main, command=self.step_sim).pack(pady=5, padx=15, fill=tk.X)
        
        # Run N Generations
        run_n_frame = ctk.CTkFrame(ctrl_card, fg_color="transparent")
        run_n_frame.pack(fill=tk.X, padx=15, pady=(10, 5))
        ctk.CTkEntry(run_n_frame, textvariable=self.num_gens_to_run, text_color=self.text_main, border_color=self.border_color, width=60).pack(side=tk.LEFT, padx=(0, 10))
        ctk.CTkButton(run_n_frame, text="Run N Gens", fg_color="transparent", border_width=1, border_color=self.border_color, text_color=self.text_main, command=self.run_n_gens).pack(side=tk.LEFT, fill=tk.X, expand=True)

        ctk.CTkCheckBox(ctrl_card, text="Auto-stop on Fixation", variable=self.stop_on_fixation_var, text_color=self.text_main, font=self.font_label).pack(pady=15, padx=15, anchor="w")

        # System Controls
        ctk.CTkButton(control_sidebar, text="Reset Simulation", fg_color="#E74C3C", hover_color="#C0392B", text_color="#FFFFFF", command=self.reset_sim).pack(pady=(10, 0), fill=tk.X)
        ctk.CTkLabel(control_sidebar, textvariable=self.gen_count_var, font=ctk.CTkFont(family="Inter", size=18, weight="bold"), text_color=("#1F6AA5", "#569CD6")).pack(pady=25)

        # --- Matplotlib Integration ---
        self.fig, (self.ax_grid, self.ax_graph) = plt.subplots(2, 1, figsize=(7, 9), gridspec_kw={'height_ratios': [1, 1.2]})
        self.fig.patch.set_facecolor(self.mpl_bg) 
        self.canvas: FigureCanvasTkAgg = FigureCanvasTkAgg(self.fig, master=plot_container)
        ...
        self.grid_colors: list[str | tuple[float, ...]] = [self.empty_cell] + self.colors
        self.cmap: mcolors.ListedColormap = mcolors.ListedColormap(self.grid_colors)
        self.norm: mcolors.BoundaryNorm = mcolors.BoundaryNorm(np.arange(-1.5, 20.5, 1), len(self.grid_colors))
    ...
    def on_mutation_toggle(self) -> None:
        if self.mutation_enabled_var.get():
            ...
            self.mut_entry.configure(state="disabled", text_color=("#AAAAAA", "#777777"))
    ...
    def update_sim_mutation_rate(self, *args: Any) -> None:
        if self.sim:
            try:
                rate = float(self.mutation_rate_var.get()) if self.mutation_enabled_var.get() else 0.0
    ...
    def refresh_plots(self) -> None:
        ...
        grid_data: np.ndarray[Any, np.dtype[np.int_]] = np.full(side * side, -1, dtype=int)
        ...
        if self.img_grid is None:
            ...
            self.ax_grid.set_title("Population Distribution", fontdict={'fontsize': 14, 'fontweight': 'bold', 'color': self.mpl_text_main}, pad=15)
        ...
        for s in range(20):
            ...
            if np.any(counts > 0) or s in self.lines_graph:
                if s not in self.lines_graph:
                    line: Line2D
                    line, = self.ax_graph.plot(gens, counts, color=self.colors[s], label=f"Sp {s}", linewidth=2)
